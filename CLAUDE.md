...
internal/oplog      GUI 操作日志：时间戳行直接输出 stdout（PowerShell 启动
                     时控制台与 GUI 并行滚动，随进程死亡自然终结；不写文件、
                     无清理逻辑，docs/09 §3.2/§3.5）
internal/nativecrash Windows 原生异常兜底：SEH 过滤器 + minidump（dbghelp
                     syscall，Install(dir)，docs/09 §3.3），其余平台 no-op
