...
- **md5 去重有效（仅限同目录）**：同内容同目录只存一份（Pic 46,891 个 Ori = 46,891 张不同原图）
- **膨胀来自多尺寸缩略图**：同一张原图生成 `_0` + `_720` 等多张（实测同 md5 计 2 次），每张独立文件
- **Thumb 占 Pic 66%** → 删 Thumb 零数据损失（Ori 仍在，QQ 可从 Ori 重建）
- **跨目录重复存在**：QQ 只按目录去重——同一文件在不同月份/目录/账号会以
  **不同的 md5 文件名**各存一份（实测存在大量字节数完全相同的文件）。
  因此工具在扫描后做**内容哈希二次扫描**（size 冲突组 → SHA-256），
  「去重建议」按真实内容分组（docs/03 §4）。结论修正为：**不做全量内容
  哈希（无冗余 I/O），只对大小冲突候选做字节级去重**
- **为什么是 SHA-256**：内容哈希是「删除哪些副本」的决策依据，必须远超
  QQ 文件名 md5 的 128 位强度（生日界 ~2^64，规模化文件集合下已有碰撞
  风险）；SHA-256（256 位 = 64 个十六进制字符）碰撞概率实际为零，且
  stdlib 自带硬件加速——只对大小冲突组计算，成本可控
