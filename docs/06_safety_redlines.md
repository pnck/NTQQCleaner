...
## 3. 删除留痕：移动与审计均为显式 opt-in（产品决策）

- **以移动代替删除**：仅当用户显式选择（GUI 确认对话框勾选「以移动代替
  删除」、CLI 传 `--backup-dir`）且已配置备份目录时才移动文件（移动 =
  天然可恢复，比删除安全）。未配置备份目录时：GUI 在确认对话框内**拒绝
  确认并引导去设置**，Go 侧同样强制校验（Move=true 且无 BackupDir →
  拒绝整次运行）；不勾选则一律直接删除
- **审计记录按需生成**（默认关——十万级文件的逐文件清单过长）：GUI 确认
  对话框勾选「生成审计记录」或 CLI 传 `--audit-log`。生成后是系统 tmp 下
  带时间戳的**临时报告**（清理后自动打开供查看/另存，不轮转不积累，
  工具自身不留垃圾）。产品决策：审计不计算 SHA-256，只记路径/大小/时间
- **清理结果回显**（每次清理，与审计无关）：统计（处理/移动/删除/跳过/
  失败/释放字节）+ 跳过/失败的逐文件明细。未启用审计时这是本次清理的
  留痕；审计文件（启用时）是完整的权威清单
- 未留痕就删除 = 程序 bug，禁止（留痕 = 上述结果回显或审计文件）

## 4. `--force` 显式参数 + 二次确认

- 没有 `--force` 参数 → 工具拒绝任何删除
- 有 `--force` → 仍须显式确认：CLI 交互输入 `yes`（打印待删数量/总大小/
  处理方式/审计选择）；GUI 在选项对话框（审计/移动 checkbox）之后必须经
  **Go 侧原生确认对话框**（确认在 Go 侧，前端不可信）
